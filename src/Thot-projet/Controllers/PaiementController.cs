    ...
    public class PaiementController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var list = db.Paiements.Where(p => p.UtilisateurId == uid)
                                   .OrderByDescending(p => p.PayeLe).ToList();
            return View(list);
        }

        // Création guidée depuis l’inscription
        ...
        public ActionResult Create(decimal? Montant, string Monnaie = "CAD", string Statut = "Payé")
        {
            ...
            ViewBag.Montant = Montant ?? 0.01m; ViewBag.Monnaie = Monnaie; ViewBag.Statut = Statut;
            return View();
        }
        ...
        public ActionResult Create(decimal Montant, string Monnaie, string Statut)
        {
            ...
            { ViewBag.Monnaies = new[] { "CAD", "USD", "EUR" }; ViewBag.Statuts = new[] { "Payé", "En attente", "Annulé" }; return View(); }

            db.Paiements.Add(new Paiement { UtilisateurId = uid, Montant = Montant, Monnaie = Monnaie.Trim(), Statut = Statut.Trim(), PayeLe = DateTime.UtcNow });
            db.SaveChanges();
            TempData["ok"] = "Paiement enregistré."; return RedirectToAction("Index");
        }
